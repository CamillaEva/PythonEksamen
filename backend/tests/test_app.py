...
def setup_function():
    empty_df = pd.DataFrame(columns=["Date", "Meal", "Food", "Gram", "Calories"])
...
def test_get_meals():
    ...
    client.post("/meals", json=new_meal)
...
def test_add_meal():
    ...
    response = client.post("/meals", json=new_meal)
...
def test_update_meal():
    ...
    response = client.put("/meals/0", json=updated_meal)

    assert response.status_code == 200
    assert response.json() == {"message": "Meal updated"}

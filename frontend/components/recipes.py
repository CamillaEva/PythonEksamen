...
def get_recipe_titles(client):
    ...
    return [recipe.strip() for recipe in answer.split("\n") if recipe.strip()]
...
def show_recipes(client):
    if "recipes" not in st.session_state:
        st.session_state["recipes"] = get_recipe_titles(client)
    ...
    for recipe in st.session_state["recipes"]:
        with st.expander(recipe):
            ...
            if recipe_key not in st.session_state:
                st.session_state[recipe_key] = get_recipe_details(client, recipe)
